...
#let template(
  ...
  // 设置正文和代码的字体
  set text(font: (fonts.en-font, fonts.zh-font), size: 13pt, lang: "zh", region: "cn")
  show raw: set text(font: fonts.code-font, 11pt)
  ...
  // set figure(
  //   numbering: (..nums) => locate(loc => {
  //     numbering("1.", chaptercounter.at(loc).first(), ..nums)
  //   })
  // )
  ...
  box(width: 100%, height: 50%)[
    ...
    #stack(
      dir: ltr,
      spacing: 16pt,
      image("./figures/vite.svg"),
      stack(
        // dir: top,
      rect(height: 36pt, width: 100%, fill: none, stroke: none),
      text(36pt ,"开源之夏")
      )
    )

    // #align(center + horizon)[
    //   #image("figures/ekuiper.png")
    // ],
    #align(right+bottom)[
      #align(left + bottom)[
        #image("figures/ekuiper.png")
  ...
  box(width: 100%, height: 40%)[
    #align(right+top)[
        ...
        box(inset: (y: 30pt), {
          authors.map(author => {
            text(20pt, weight: "semibold")[
              #if "homepage" in author {
                [#link(author.homepage)[#author.name]]
              } else { "申请人："+author.name }]
            ...
            if "email" in author {
              link("mailto:" + author.email, box(height: 1.1em, baseline: 13.5%)[#image("./figures/envelope-solid.svg")])
            }
          }).join(", ", last: {
            if authors.len() > 2 {
              ", and"
